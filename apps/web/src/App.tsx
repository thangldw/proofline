...
export function SearchView({onEvidence, sources = []}: {onEvidence: (item: Evidence, title: string) => void; sources?: Source[]}) {
  ...
  return <section className="content search-view">
    ...
    {contextBudgetExclusions.length > 0 && <div className="context-budget-notice" role="status" aria-label="Context budget notice"><strong>{contextBudgetExclusions.length} retrieved {contextBudgetExclusions.length === 1 ? "span was" : "spans were"} excluded by the context budget.</strong><span>Evidence IDs: {contextBudgetExclusions.map(exclusion => <code title={exclusion.evidence_id} key={exclusion.evidence_id}>{exclusion.evidence_id.slice(0, 8)}</code>)}</span></div>}
...
function SearchHitCard({hit, onEvidence}: {hit: SearchHit; onEvidence: (item: Evidence, title: string) => void}) {
  return <article className="result-card"><div className="result-meta"><span>{hit.source_title}</span><button onClick={() => onEvidence({id: hit.chunk_id, source_id: hit.source_id, source_version_id: hit.source_version_id, quote: hit.content, start_offset: hit.start_offset, end_offset: hit.end_offset, start_line: hit.start_line, end_line: hit.end_line}, hit.source_title)}>Lines {hit.start_line}–{hit.end_line}</button></div><p>{hit.content}</p><details className="retrieval-debug"><summary>Why this result?</summary><dl><div><dt>Channels</dt><dd>{hit.retrieval_channels.join(" + ")}</dd></div>{hit.lexical_rank !== null && <div><dt>Lexical rank</dt><dd>#{hit.lexical_rank}</dd></div>}{hit.semantic_rank !== null && <div><dt>Semantic rank</dt><dd>#{hit.semantic_rank}</dd></div>}{hit.semantic_score !== null && <div><dt>Semantic score</dt><dd>{hit.semantic_score.toFixed(4)}</dd></div>}{hit.fused_score !== null && <div><dt>RRF score</dt><dd>{hit.fused_score.toFixed(4)}</dd></div>}<div><dt>Source version</dt><dd><code title={hit.source_version_id}>{hit.source_version_id.slice(0, 8)}</code></dd></div><div><dt>Exact location</dt><dd>Lines {hit.start_line}–{hit.end_line} · offsets {hit.start_offset}:{hit.end_offset}</dd></div></dl></details></article>;
}
