...
from .schemas import DecisionImpactRead, DecisionImpactSnapshot, DecisionImpactSummary
...
def _summary(
    findings: list[DecisionImpactFinding], evaluated_at: datetime
...
def decision_impact_summary(
    ...
    return _summary(findings, evaluated_at)


@router.get("/decision-impacts/snapshot", response_model=DecisionImpactSnapshot)
def decision_impact_snapshot(
    as_of: datetime | None = Query(default=None),
    workspace_id: str = Depends(resolve_workspace_id),
    session: Session = Depends(get_session),
) -> DecisionImpactSnapshot:
    findings, evaluated_at = _findings(session, workspace_id, as_of)
    return DecisionImpactSnapshot(
        findings=[DecisionImpactRead.model_validate(item.model_dump()) for item in findings],
        summary=_summary(findings, evaluated_at),
    )
