...
Use `make setup`, `make test`, `make check` and `npm run test:e2e` before release. Back up the local Proofline home before migration or import, verify the backup, and restore into a separate location first. Release `v1.0.1` only from a clean commit after `scripts/release_check.py --tag v1.0.1` passes.
